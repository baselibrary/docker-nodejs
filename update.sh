...
for version in "${versions[@]}"; do	
  if [ $version == "4" ]; then
    ...
    fullVersion="$(curl -fsSL "https://deb.nodesource.com/node_5.x/dists/trusty/main/binary-amd64/Packages.gz" | gunzip | awk -F ': ' '$1 == "Package" { pkg = $2 } pkg ~ /^nodejs$/ && $1 == "Version" { print $2 }' | sort -rV | head -n1 )"
  	...
  	fullPackage="nodejs"
    fullVersion="$(curl -fsSL "https://deb.nodesource.com/node_6.x/dists/trusty/main/binary-amd64/Packages.gz" | gunzip | awk -F ': ' '$1 == "Package" { pkg = $2 } pkg ~ /^nodejs$/ && $1 == "Version" { print $2 }' | sort -rV | head -n1 )"
	fi
	(
		set -x
		sed '
